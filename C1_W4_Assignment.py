...
def get_matrices(en_fr, french_vecs, english_vecs):
    ...
    for en_word, fr_word in en_fr.items():
        ...
        # check that both words have embeddings
...
def compute_loss(X, Y, R):
    ...
    Outputs:
        loss: scalar - the value of the loss function for given X, Y and R.
    '''
    ### START CODE HERE ###
...
def align_embeddings(X, Y, train_steps=100, learning_rate=0.0003, verbose=True, compute_loss=compute_loss, compute_gradient=compute_gradient):
    ...
    Inputs:
        X: a matrix of dimension (m,n) where the rows are the English embeddings.
        Y: a matrix of dimension (m,n) where the rows correspond to the French embeddings.
        train_steps: positive int - number of gradient descent iterations.
    ...
    # R starts as a random square matrix (dimensions = embedding size)
...
def nearest_neighbor(v, candidates, k=1, cosine_similarity=cosine_similarity):
    """
    Input:
      - v: the vector you want to find the nearest neighbor for
    ...
    # Reverse the order of the sorted_ids array (to get descending similarity)
...
def test_vocabulary(X, Y, R, nearest_neighbor=nearest_neighbor):
    ...
    Output:
        accuracy: accuracy of translations from English to French
    '''
    import numpy as np

    ### START CODE HERE ###
    # The prediction is X times R (transform English embeddings to French space)
    pred = np.dot(X, R)
    ...
    for i in range(len(pred)):
        # get the index of the nearest neighbor of pred[i]; candidates are Y
        pred_idx = nearest_neighbor(pred[i], Y, k=1)[0]

        # if the index of the nearest neighbor equals the row index
        if pred_idx == i:
...
def get_document_vecs(all_docs, en_embeddings, get_document_embedding=get_document_embedding):
    ...
    # dictionary: key = index of doc, value = embedding
    ind2Doc_dict = {}
    ...
    for i, doc in enumerate(all_docs):
        ...
        # save the document embedding into the dictionary
...
def hash_value_of_vector(v, planes):
    ...
    ### START CODE HERE ###
    # dot product between v and planes → shape (1, N_PLANES)
    dot_product = np.dot(v, planes)

    # sign: negative → 0, positive or zero → 1
    h = (dot_product >= 0).astype(int)
...
def compute_loss(X, Y, R):
    ...
    Inputs: 
        ...
        R: a matrix of dimension (n,n) - transformation matrix from English to French vector space embeddings.
    Outputs:
        loss: scalar value - the value of the loss function for given X, Y and R.
    '''
    ### START CODE HERE ###
    # m is the number of examples (rows in X)
    m = X.shape[0]
    ...
    # sum_diff_squared is the sum of all squared elements
    sum_diff_squared = np.sum(diff_squared)

    # loss is the average squared difference
    loss = sum_diff_squared / m
    ### END CODE HERE ###
    return loss
...
def align_embeddings(X, Y, train_steps=100, learning_rate=0.0003, verbose=True, compute_loss=compute_loss, compute_gradient=compute_gradient):
    ...
    # Initialize R as a random square matrix of size (embedding_dim, embedding_dim)
    R = np.random.rand(X.shape[1], X.shape[1])
...
def nearest_neighbor(v, candidates, k=1, cosine_similarity=cosine_similarity):
    ...
    # sort the similarity list and get the indices (ascending by default)
    sorted_ids = np.argsort(similarity_l)
...
def test_vocabulary(X, Y, R, nearest_neighbor=nearest_neighbor):
    ...
    Input:
        ...
        R: the transform matrix which translates word embeddings from
           English to French word vector space.
    Output:
        accuracy: fraction of correctly aligned words
    '''
    ...
    # loop through each row in pred (each transformed embedding)
    for i in range(len(pred)):
        # get the index of the nearest neighbor of pred[i]; also pass in Y
        pred_idx = nearest_neighbor(pred[i], Y, k=1)[0]
    ...
    # accuracy is the number correct divided by total number of rows
    accuracy = num_correct / len(pred)
    ### END CODE HERE ###
...
def get_document_vecs(all_docs, en_embeddings, get_document_embedding=get_document_embedding):
    ...
    for i, doc in enumerate(all_docs):
        ...
        # save the document embedding into the ind2Doc_dict dictionary at index i
        ind2Doc_dict[i] = doc_embedding
...
def get_document_vecs(all_docs, en_embeddings, get_document_embedding=get_document_embedding):
    ...
    for i, doc in enumerate(all_docs):
        ...
        # save the document embedding into the dictionary at index i
        ind2Doc_dict[i] = doc_embedding
...
def hash_value_of_vector(v, planes):
    ...
    """
    ### START CODE HERE ###
    # calculate dot product between the vector and planes
    dot_product = np.dot(v, planes)  # shape (1, N_PLANES)

    # get the sign of the dot product
    sign_of_dot_product = np.sign(dot_product)

    # set h = 1 if positive or zero, 0 if negative
...
def approximate_knn(doc_id, v, planes_l, hash_tables, id_tables, k=1, num_universes_to_use=25, hash_value_of_vector=hash_value_of_vector):
    ...
    for universe_id in range(num_universes_to_use):
        ...
        for i, new_id in enumerate(new_ids_to_consider):
            ...
            if new_id not in ids_to_consider_set:
        # get the document vector at index i
                document_vector_at_i = document_vectors_l[i]
                vecs_to_consider_l.append(document_vector_at_i)
